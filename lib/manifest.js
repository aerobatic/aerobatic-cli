    ...
    .then(exists => {
      if (exists) {
        throw Error.create('There is already a ' + chalk.bold(FILENAME) + ' file in this directory. ' +
          ...
          'key and re-run ' + chalk.green.underline('aero create') + '.', {formatted: true});
      }
      return;
    });
}
  ...
  return fs.exists(filePath)
    .then(exists => {
      if (!exists) {
        throw Error.create('There is no ' + chalk.bold(FILENAME) + ' file in this directory. ' +
          'First run ' + chalk.green.underline('aero create') + '.', {formatted: true});
      }
    })
    .then(() => {
    ...
    .then(yamlString => {
      ...
      if (!appManifest.appId) {
        throw Error.create('There is no ' + chalk.bold('appId') +
        ' value in ' + chalk.bold(FILENAME) + '.\n' +
        'First run ' + chalk.underline.green('aero create') + '.', {formatted: true});
      }

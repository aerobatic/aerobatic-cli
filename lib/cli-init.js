    ...
    initPromise = initPromise.then(() => {
      ...
      return api.get(requestOptions)
        .then(website => {
          ...
          if (program.customerId && website.customerId !== program.customerId) {
            throw Error.create('This website belongs to a different account. You might need to run the ' +
              chalk.green.underline('aero switch') + ' command.', {formatted: true});
          }
